...
```sh npm2yarn copy
npm install @a21/apl-sdk @saturnbtcio/arch-sdk
```
...
The sdk also provides methods to create APL transactions. In the example below we transfer a token from one account to another.

```typescript filename="transfer-token.ts" copy
